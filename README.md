...
Скрипт берёт билд API так: сначала ищет рядом с собой каталог `./api` (где лежит
`MCHSWebAPI.dll`), а если не находит — скачивает архив `api_linux.7z` с GitHub Release.

### Шаг 1. Соберите билд API (на машине разработчика)
...
В каталоге `publish/api` появится `MCHSWebAPI.dll`. Запакуйте его при необходимости:

```bash
7z a api_linux.7z ./publish/api/*        # или: tar -czf api_linux.tar.gz -C publish/api .
```

### Шаг 2. Установка на сервере

Зайдите на VPS по SSH под root (или через `sudo`), положите скрипт и распакованный билд
рядом (билд — в подкаталог `api/`), затем запустите:

**Ubuntu / Debian / Astra Linux**

```bash
mkdir -p api && 7z x -y -oapi api_linux.7z   # распаковать билд в ./api
sudo bash install-deb.sh                      # на вопрос выберите 1 (Переустановить)
```

**RedOS / RHEL-совместимые**

```bash
mkdir -p api && 7z x -y -oapi api_linux.7z
sudo bash install-rpm.sh
```

> Если каталога `./api` рядом нет, скрипт скачает билд с GitHub Release по ссылке `API_URL`
> из своего заголовка — тогда достаточно только запустить `sudo bash install-deb.sh`.
